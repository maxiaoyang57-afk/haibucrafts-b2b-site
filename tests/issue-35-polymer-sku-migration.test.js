...
test('Issue #35 mappings remain intact across 12 active Polymer identities', async () => {
  ...
  assert.equal(polymer.length, 30);
  assert.equal(new Set(polymer.map((product) => product.sku)).size, 30);
  assert.equal(category.match(/class="product-card-v2"/g)?.length, 30);
